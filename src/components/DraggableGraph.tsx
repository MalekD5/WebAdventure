...
export default function DraggableGraph() {
	...
	return (
		<div className="flex flex-col items-center gap-4 min-h-screen bg-gray-900 p-8">
			...
			{isCorrect !== null && (
				<div
					className={`mt-4 p-4 rounded-lg ${
						...
						isCorrect !== null ? "scale-100" : "scale-0"
					}`}
				>
					{isCorrect ? (
						<p className="flex items-center gap-2">
							<span className="text-xl">✓</span> Correct sequence! The refueling
							system is properly connected.
						</p>
					) : (
						<p className="flex items-center gap-2">
							<span className="text-xl">✗</span> Incorrect sequence. Try
							connecting the nodes in a different order.
						</p>
					)}
				</div>
			)}
